...
- `settings.properties` — 既定の設定。**11 アルゴリズム**の ON/OFF と 32 個のパラメータ、**`[timezone]`(全て UTC。※CR-008)**、**`[chart]`(※CR-009)**
- `VERSION` — バージョン文字列(現在 0.5.0)
...
- `discovery.py` — M03。`{dir}` の再帰走査と 3 パターンのファイル名判定
- `errors.py` — `AppError` と 7 つの派生。終了コードとの対応表 `EXIT_CODE_MAP` を持つ
- `schema.py` — DuckDB の DDL・論理主キー・列型。冪等
- `metrics.py` — M07。3 テーブル → 統一テーブル `metrics`。segment 分割・累積値の差分・使用率の導出
