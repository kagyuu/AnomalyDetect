...
└── app/                       ← ソースツリー(配布単位)
    ...
    └── src/
        └── s_anomaly/
            ...
            ├── charts.py         M15 SVG チャート(※CR-009。※P012 #6 によりツリーへ補記)
...
**DS-02-05** ※CR-010: **`[sar] activities` はカンマ区切りの文字列として受け取り、単項検証で語ごとに検査する。**

* 空白を除去し、空要素を捨てる。**大文字小文字は区別しない**(内部では小文字に正規化する)。
* **既知の 8 語以外は WARNING を出して無視する**(DS-02-04 と同じ扱い。終了コード 3 にはしない)。設定ミスで実行が止まるより、**「その活動が取り込まれていない」ことがレポートの系列数から分かる**ほうがよい。
* **全部が未知だった場合・空文字だった場合は、sar を 1 件も取り込まない。** これはエラーではない(`sa-*.csv` が 1 件も無い場合と同じ状態になる)。
* 既定値は 8 語すべて。`Config` は正規化済みの `frozenset` として保持し、**M03(探索)ではなく M16(ローダ)が見出し行の判別時に参照する**。★探索の時点では活動種別が分からない(ファイル名に書かれていない)ため、絞り込みはローダ側でしか行えない。★
...
**DS-03-02a** ※CR-010: ④ の `(.+)` は貪欲一致であり、**ホスト名に `-` が含まれていても正しく分割できる**(`sa-host-01-20260601.csv` → ホスト名 `host-01`、日付 `20260601`)。末尾の 8 桁数字が日付であることが分割位置を一意に決める。日付部が 8 桁数字でない場合は種別なしとして読み飛ばす。
...
## 5. M04〜M06・M16 `loaders` — 解析と格納 ※CR-010により M16 を追加
...
### 5.4a M16 `loaders.sar` ※CR-010により新設

**DS-SAR-01**: ファイルは**活動種別ごとのブロックの連なり**である。`#` で始まる行が見出しで、それ以降の行が直前の見出しに属する。**見出しに出会うたびに現在の列マップを差し替える。**
...
**DS-SAR-02**: **活動種別は、見出し行の第 4 列以降の列名の集合から判別する。** ファイルに活動名そのものは書かれていないためである。判別は**先頭 1 列(デバイス列)と特徴列 1 つ**の組で行う。

| 活動 | 判別条件(見出しの第 4 列以降) | デバイス列 |
| --- | --- | --- |
| `cpu` | `CPU` で始まり `%user` を含む | `CPU` |
| `memory` | `kbmemfree` を含む | なし |
| `swap_space` | `kbswpfree` を含む | なし |
| `io` | `tps` を含み `rtps` を含む | なし |
| `load` | `runq-sz` を含む | なし |
| `swapping` | `pswpin/s` を含む | なし |
| `disk` | `DEV` で始まり `%util` を含む | `DEV` |
| `network` | `IFACE` で始まり `rxpck/s` を含む | `IFACE` |
| **`nfs`** ※CR-011 | **`retrans/s` を含む** | なし |
| **`nfsd`** ※CR-011 | **`scall/s` を含む** | なし |

**DS-SAR-02a** ※CR-011: **`nfs` と `nfsd` の判別列は、互いに衝突しないものを選ぶ。**
`-n NFS` は `read/s` / `write/s` / `access/s` を持ち、`-n NFSD` は `sread/s` / `swrite/s` /
`saccess/s` を持つ。**接頭の `s` があるかどうかだけの違いであり、部分一致で判別すると取り違える。**
正規化した名前の**完全一致**で判別しているため問題は起きないが、
**判別条件を「`read_s` を含む」のような弱いものに変えてはならない。**
`retrans_s`(NFS 固有)と `scall_s`(NFSD 固有)は、**どちらか一方にしか現れない。**

**どの条件にも一致しない見出しは、そのブロック全体を読み飛ばす**(P001 FR-141)。読み飛ばした見出し行は**見出しの文字列ごとに 1 回だけ** WARNING に出す(1 日分で同じ見出しが 288 回現れるため、行ごとに出すとログが埋まる)。

★ACCEPTED★ **判別を「列名の集合」で行い、活動名を書かせない。** **検討したこと**: 変換スクリプト側で活動名の行(`# activity: cpu` など)を挿入する案。**採らなかった理由**: `sadf` の出力そのものを加工することになり、「人が採取ホスト上で `sadf` を流すだけ」という前処理の単純さが失われる。既に配布済みのスクリプトとの互換も切れる。**残存リスク**: sysstat の版差で特徴列名が変わると判別に失敗する(`rd_sec/s` → `rkB/s` のような改称が実在する)。**判別に使う列は改称の実績が無いものを選んでいる**(`%user` / `kbmemfree` / `kbswpfree` / `rtps` / `runq-sz` / `pswpin/s` / `%util` / `rxpck/s`)が、将来の改称は防げない。**判別できないブロックは黙って消えるのではなく WARNING に出る**ため、気付ける。

**DS-SAR-03**: **列は位置ではなく列名で対応付ける。** 見出し行から `{正規化した列名: 列位置}` の対応表を作り、**取り込み対象のメトリクス(P001 FR-142 の表)だけ**を拾う。対象外の列は読み飛ばす(WARNING は出さない。取り込まないことは設計どおりであり、異常ではない)。

**DS-SAR-04**: 列名の正規化(P001 FR-143)。
...
**版差の吸収**は、正規化した名前に対する**別名表**で行う。

| 正規の名前 | 受理する別名(sysstat の版差) |
| --- | --- |
| `rxkb_s` | `rxkB_s` の正規化結果(`rxkb_s`。小文字化により自動的に一致する) |
| `bread_s` | `breadn_s` は**別物なので受理しない** |
| `await` | `await` のみ。新しい版の `r_await` / `w_await` は取り込まない(★FIXME★ 取り込むべきかは実データで確認が要る) |

★訂正の余地★ `-d -p` の `rd_sec/s` → `rkB/s`、`avgqu-sz` → `aqu-sz` の改称は、**いずれも取り込み対象のメトリクスに含めていない**(取り込むのは `%util` / `await` / `tps` の 3 つ)。したがって**本CRの範囲では版差の影響を受けない。** 将来これらを取り込む場合に別名表が要る。

**DS-SAR-05**: **時刻とタイムゾーン表記**(P001 FR-147)。

```python
# "2026-06-01 00:10:00 UTC" -> (datetime, "UTC")
m = re.match(r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})(?:\s+(\S+))?$", token)
```

* 第 2 群(タイムゾーン表記)が取れた場合、`[timezone] sar` の設定値と**文字列として比較**する。食い違えば**ファイル単位で 1 回だけ** WARNING を出す。
* **比較は表記の同一性だけを見る。** `UTC` と `Etc/UTC` のような等価判定は行わない。★ACCEPTED★ **検討したこと**: IANA 名の別名解決を行う案。**採らなかった理由**: 別名表を持つ必要があり、DuckDB へ問い合わせる経路も S4 の内側では重い。**警告であって停止ではない**ため、偽陽性の害が小さい。**残存リスク**: `Etc/UTC` を設定した環境では毎回 WARNING が出る(動作は正しい)。
* **表記が無い場合は警告を出さない。** sysstat の版によっては付かないためである。
* **設定値を正として変換する**(DS-LD-TZ と同じ経路。CR-008)。表記を優先した自動変換はしない。

**DS-SAR-06**: ホスト名は**ファイル名から取る**(P001 FR-146)。データ行の第 1 列にもホスト名があるが使わない。**両者が食い違っても WARNING を出さない。** ★ACCEPTED★ **検討したこと**: 食い違いを警告する案。**採らなかった理由**: 変換スクリプトは「他のログに名前を揃えるために」ホスト名を引数で上書きできる仕様であり(P001 FR-145)、**食い違うことが正常な使い方である**。警告を出すと、正しく運用しているときに毎回警告が出る。**残存リスク**: ファイル名のホスト名を書き間違えても気付けない。

**DS-SAR-07**: 1 データ行から、**取り込み対象の列の数だけレコードを生成する**(横持ち → 縦持ち)。`[sar] activities` に含まれない活動のブロックは、**見出しの時点で判別し、データ行を解析せずに読み飛ばす**(解析してから捨てるより速い)。

**DS-SAR-08**: 挿入は他のローダと同じく一時 CSV + `read_csv`(ADR-011)で行う。**縦持ちのため 1 データ行が最大 4 レコードに増える**(`cpu` は 4 メトリクス)。バッチ境界の判定は**生成したレコード数**で行う(読んだ行数ではない)。
...
**DS-LD-06a** ※CR-010: **`sar` の論理 PK は 5 列**(`ts` / `host` / `activity` / `device` / `metric`)である。縦持ちのため、**メトリクス名まで含めないと 1 行を特定できない**。他の 3 テーブルと同じ `row_number()` の経路に載せるが、`PARTITION BY` の列だけが異なる。

```sql
PARTITION BY ts, host, activity, device, metric ORDER BY _load_seq
```
...
### 6.3a `sar` の合流 ※CR-010により新設

**DS-07-07a**: `sar` は**格納時点で既に縦持ち**であるため、`metrics` へは列名の付け替えだけで合流する。GC のような導出も単位変換も要らない。
...
* **`segment` は常に 0 とする。** sar には JVM 再起動のような累積値のリセットが無い(`sadf` が出すのは瞬間値・平均値であり累積値ではない)。
* **`value` が NULL の行は既存の `WHERE value IS NOT NULL` で落ちる**(P001 FR-015 と同じ扱い)。

**DS-07-07b**: **`sar` の比率メトリクス(`pct_*`)は `DETECTABLE_METRICS` に入れる。**

**★これは静かに壊れる箇所である。** `metrics.DETECTABLE_METRICS` は**ハードコードの許可リスト**であり、ここに無いメトリクスは `list_series` が返さず、**11 アルゴリズムのどれも走らない。例外も警告も出ない。** ADR-005 が定める「`*_pct` は検知対象に含めない」は**本アプリが導出した使用率**(`ou_pct` など)についての規則であり、**sar が直接出力する比率には当てはまらない**(P001 FR-143 の表)。`RATIO_METRICS` 側に入れると、sar の主要メトリクスが**1 つも検知されないまま正常終了する。**

**この点は受け入れテストで証跡を確認する**(「例外が出ない」だけのテストにしない)。`docs/CLAUDE.md` が記録する再発型の欠陥(CR-002・CR-009 で 2 回発生)と同じ型である。
...
| **sar の比率(`pct_*`)** ※CR-010 | **そのメトリクス自身** | **100** | **いいえ** |
| 上記以外(`active_connections`, `pend` など) | そのメトリクス | 期間中の最大値 | **はい** |

**DS-08-C1-01a** ※CR-010: **sar の比率メトリクスは、それ自身が 0〜100 の比率である。**
`ou` → `ou_pct` のような**別メトリクスへの読み替えを伴わない**点が既存の 2 経路と異なるため、
実装では `RATIO_OF`(読み替え表)とは別に **`SELF_RATIO_METRICS`(それ自身が比率であるメトリクスの集合)**を持つ。
`RATIO_OF` に `pct_util → pct_util` のような自己写像を入れる案もあるが、
**「読み替え先の点数が足りるか」を確認する既存の分岐を無意味に通る**ため分けている。

| 集合 | メトリクス |
| --- | --- |
| `SELF_RATIO_METRICS` | `pct_user` / `pct_system` / `pct_iowait` / `pct_memused` / `pct_commit` / `pct_swpused` / `pct_util` / `pct_ifutil` |

**`pct_idle` は含めない。** 含めると**閑散なホストが全件「上限に張り付き」になる**(FR-148)。
...
| `lsf_queue` | `njobs`, `run` |
| **`sar`** ※CR-010 | **`pct_idle`**(100% は遊休であり正常)、`kbmemfree`(小さいほど逼迫。向きが逆)、`tps`, `bread_s`, `bwrtn_s`, `pswpin_s`, `pswpout_s`, `runq_sz`, `ldavg_1`, `ldavg_5`, `blocked`, `await`, `rxkb_s`, `txkb_s`(上限が自明でない) |
| **`sar`(NFS)** ※CR-011 | `call_s`, `retrans_s`, `read_s`, `write_s`, `access_s`, `getatt_s`, `scall_s`, `badcall_s`, `hit_s`, `miss_s`, `sread_s`, `swrite_s`, `saccess_s`, `sgetatt_s`。**すべて毎秒のレート値であり上限が自明でない** |
...
**DS-08-C1-03a** ※CR-010: **上記 2 段のガードは、sar の比率メトリクスにも同じように適用する。**
そのため、**常時 99〜100% で張り付いている系列は `min_spread`(1.5 倍)を満たさず検知されない。**
★ACCEPTED★ **検討したこと**: 上限が自明な系列(`SELF_RATIO_METRICS`)ではガード 2 を外す案。
**採らなかった理由**: ガード 2 は「推定上限が実測最大値と一致してしまう」ことへの対策だが、
**「期間中ずっと 100% だった」系列を検知したいのも事実**である。しかし本アプリは
**期間全体を 1 つの窓として見る**ため、期間の最初から最後まで張り付いた系列は
「いつから張り付いたか」を示せず、レポートの「開始/終了」が期間全体になる。
**この形の出力は、既に CR-005 の時点で `active_connections` について同じ判断をしている。**
**残存リスク**: **常に飽和しているデバイス(例: 恒常的に 100% の `%util`)は検知されない。**
それは異常ではなく定常状態であり、本アプリの守備範囲(変化と張り付きの検知)の外にある。
**運用側が「ずっと 100%」を知る手段は別に要る。** ★FIXME★ (この割り切りでよいかは実データで確認が要る)
...
| **ADR-022** ※CR-010(P905 が番号を確定) | **sar が直接出力する比率(`pct_*`)は一次の検知対象とする。ADR-005 の「`*_pct` は検知対象に含めない」は、本アプリが導出した使用率についての規則である** | `metrics.DETECTABLE_METRICS` はハードコードの許可リストであり、`RATIO_METRICS` 側に入れると**例外も警告も出ないまま 1 件も検知されない**。接頭 `pct_` と接尾 `_pct` を書き分けて区別する | 本書 DS-07-07b, DS-08-C1-01a |
| **ADR-023** ※CR-010(P905 が番号を確定) | **`sar` テーブルは縦持ち(long)で持つ。活動種別ごとの横持ちテーブルを作らない** | `sadf` の列は sysstat の版で増減するため、横持ちにすると版が変わるたびにテーブル定義の変更が要る。`metrics` へ載せる段階でどのみち縦持ちに畳む | 本書 5.4a, P002 6.2.3a |
